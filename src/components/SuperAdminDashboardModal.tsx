    ...
    <div className="fixed inset-0 z-50 bg-white text-gray-900">
      <div className="flex h-full min-h-0 flex-col">
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <h2 className="truncate text-base font-black tracking-tight text-black sm:text-lg">Super Admin Dashboard</h2>
                <p className="truncate text-[11px] text-gray-500">Platform-wide overview</p>
              </div>
            </div>
        ...
        <main className="min-w-0 flex-1 overflow-y-auto p-4 sm:p-6 xl:p-8">
          {loading ? (
            <div className="flex items-center justify-center py-20 text-sm font-bold text-gray-600"><Loader2 className="w-5 h-5 animate-spin text-black mr-2" />Loading platform data...</div>
          ) : (
            <div className="mx-auto w-full max-w-[1800px] space-y-6">
              <div className="grid gap-4 sm:grid-cols-3">
                <div className="rounded-3xl border border-gray-200 bg-white p-5"><Globe className="w-5 h-5 text-black" /><p className="mt-4 text-3xl font-black text-black">{stats.institutions}</p><p className="text-xs font-bold text-gray-400">Active Institutions</p></div>
                <div className="rounded-3xl border border-gray-200 bg-white p-5"><ClipboardList className="w-5 h-5 text-black" /><p className="mt-4 text-3xl font-black text-black">{stats.orders}</p><p className="text-xs font-bold text-gray-400">Total Orders</p></div>
                <div className="rounded-3xl border border-gray-200 bg-white p-5"><Users className="w-5 h-5 text-black" /><p className="mt-4 text-3xl font-black text-black">{stats.profiles}</p><p className="text-xs font-bold text-gray-400">Registered Users</p></div>
              </div>
              <section className="space-y-4">
                <h3 className="text-sm font-extrabold text-black">Recent Orders</h3>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {recentOrders.map((o) => (
                      <div key={o.id} className="rounded-2xl border border-gray-200 bg-white p-4">
                        <div className="flex items-center justify-between"><p className="text-xs font-black text-emerald-300">{o.order_id}</p><span className="text-[10px] font-mono text-gray-500">{new Date(o.created_at).toLocaleDateString()}</span></div>
                        ...
                        <span className={`mt-2 inline-flex rounded-full border px-2 py-0.5 text-[10px] font-black ${o.status === 'completed' ? 'text-emerald-300 border-black/40 bg-emerald-950/40' : 'text-yellow-300 border-yellow-500/40 bg-yellow-950/40'}`}>{o.status}</span>
                      </div>
                    ))}
                  </div>
                )}
