...
import type { Profile, InstitutionData } from '../../types';
...
export const ProfileTab: React.FC<ProfileTabProps> = ({
  ...
  const detailRows: Array<{ label: string; value: string; icon: React.ReactNode }> = [
    ...
    ...(!isVisitor && hasValue(profile?.registration_id) ? [{ label: 'Registration ID', value: profile!.registration_id!, icon: <Hash className="w-4 h-4" /> }] : []),
    ...(!isVisitor && hasValue(profile?.student_id) ? [{ label: 'Student ID', value: profile!.student_id!, icon: <Shield className="w-4 h-4" /> }] : []),
