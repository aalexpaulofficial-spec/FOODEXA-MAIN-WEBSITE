          ...
          <div className="grid lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-6 space-y-6">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-950 text-indigo-300 border border-indigo-800 text-xs font-mono">
                ...
                <span>University ROI Calculator</span>
              </div>

              <h3 className="text-2xl sm:text-3xl font-extrabold text-white">
                Calculate Your Campus Efficiency Projection
              </h3>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-xs sm:text-sm font-semibold text-slate-300">
                    <span>Total Campus Student Population:</span>
                    <span className="text-emerald-400 font-mono font-bold text-base">{studentCount.toLocaleString()} students</span>
                  </div>
                  <input
                    type="range"
                    min={2000}
                ...
                {/* Canteen Count Slider */}
                <div className="space-y-2">
                  <div className="flex justify-between text-xs sm:text-sm font-semibold text-slate-300">
                    <span>Active Campus Dining Outlets / Cafes:</span>
                    <span className="text-emerald-400 font-mono font-bold text-base">{canteenCount} outlets</span>
                  </div>
                  <input
                    type="range"
                    min={2}
                    ...
                    step={1}
                    value={canteenCount}
                    onChange={(e) => setCanteenCount(Number(e.target.value))}
                    className="w-full accent-emerald-400 cursor-pointer h-2 bg-slate-950 rounded-lg"
                  />
                </div>
              </div>
              ...
              <p className="text-xs text-slate-400">
                Projections based on FOODEXA deployment benchmark studies across 45+ partner universities.
              </p>
            </div>

            {/* Right Output */}
            <div className="lg:col-span-6 bg-slate-950 rounded-2xl p-6 border border-slate-800 space-y-6 text-center sm:text-left">
              <h4 className="text-xs font-mono text-slate-400 uppercase tracking-wider text-center">
                ANNUAL CAMPUS IMPACT PROJECTION
              </h4>
              ...
              <div className="space-y-4">
                <div className="bg-slate-900 p-4 rounded-2xl border border-emerald-500/30">
                  ...
                  <div className="text-3xl font-black text-emerald-400 font-mono mt-1">
                    +${estimatedAnnualVendorRevenueIncrease.toLocaleString()} / yr
                  </div>
                  <div className="text-[10px] text-slate-500 mt-1">From higher throughput during peak 45-minute lunch gaps</div>
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-slate-900 p-3.5 rounded-2xl border border-teal-500/30">
                    <div className="text-[10px] text-slate-400 font-mono">Perishable Waste Saved</div>
                    <div className="text-xl font-bold text-teal-300 font-mono mt-0.5">
                      {estimatedFoodWasteReducedTons} Tons / yr
                    </div>
                  </div>

                  <div className="bg-slate-900 p-3.5 rounded-2xl border border-indigo-500/30">
                    <div className="text-[10px] text-slate-400 font-mono">Student Hours Saved</div>
                    <div className="text-xl font-bold text-indigo-300 font-mono mt-0.5">
                      {totalStudentHoursSavedPerYear.toLocaleString()} hrs
                    </div>
                  </div>
                </div>
              </div>
              ...
              >
                <span>Request Custom Campus Report</span>
