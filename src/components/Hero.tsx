...
import { Sparkles, ArrowRight, Download, Building2, Mic, QrCode, Search, ShoppingBag, Star, Clock, Lock, ExternalLink } from 'lucide-react';

const HERO_STATS = [
  { value: '250,000+', label: 'Daily Campus Meals' },
  { value: '180+', label: 'Institutions Registered' },
    ...
    <section className="relative pt-28 pb-16 md:pt-36 md:pb-24 overflow-hidden bg-slate-950">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-8 items-center">
          ...
          <div className="lg:col-span-7 space-y-8 text-center lg:text-left">
            ...
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-slate-900/90 border border-emerald-500/30 shadow-inner">
              ...
              <span className="text-xs font-mono text-emerald-300">
                Official Campus Food Ordering SaaS
              </span>
            ...
            <div className="flex flex-wrap items-center justify-center lg:justify-start gap-3">
              ...
              <button
                onClick={onOpenRegisterInstitution || onOpenGetStarted}
                className="px-6 py-3.5 rounded-2xl bg-gradient-to-r from-emerald-400 via-teal-300 to-cyan-400 text-slate-950 font-extrabold text-xs hover:from-emerald-300 hover:to-cyan-300 transition-all shadow-lg shadow-emerald-500/25 hover:shadow-emerald-500/40 flex items-center justify-center gap-2 cursor-pointer"
              >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
              <div className="bg-slate-900/80 border border-slate-800/90 rounded-2xl p-3.5 text-left space-y-1.5 hover:border-emerald-500/40 transition-colors">
                ...
                <h3 className="text-xs font-bold text-white">Voice Ordering</h3>
                <p className="text-[11px] text-slate-400 leading-tight">Order meals naturally using AI voice</p>
              </div>

              <div className="bg-slate-900/80 border border-slate-800/90 rounded-2xl p-3.5 text-left space-y-1.5 hover:border-emerald-500/40 transition-colors">
                <div className="text-xl">📱</div>
                <h3 className="text-xs font-bold text-white">QR Pickup</h3>
                <p className="text-[11px] text-slate-400 leading-tight">Instant contactless locker & counter pickup</p>
              </div>

              <div className="bg-slate-900/80 border border-slate-800/90 rounded-2xl p-3.5 text-left space-y-1.5 hover:border-emerald-500/40 transition-colors">
                <div className="text-xl">⚡</div>
                <h3 className="text-xs font-bold text-white">Live Tracking</h3>
                <p className="text-[11px] text-slate-400 leading-tight">Real-time prep updates & locker codes</p>
              </div>

              <div className="bg-slate-900/80 border border-slate-800/90 rounded-2xl p-3.5 text-left space-y-1.5 hover:border-emerald-500/40 transition-colors">
                <div className="text-xl">🤖</div>
                <h3 className="text-xs font-bold text-white">LX Assistant</h3>
                <p className="text-[11px] text-slate-400 leading-tight">Smart meal discovery & dietary filters</p>
              </div>
            </div>
          ...
          {/* Right Hero Column - Student Food Ordering Dashboard Preview */}
          <div className="lg:col-span-5 relative">
            ...
            <div className="bg-slate-900/95 rounded-3xl p-5 border border-slate-800 shadow-2xl shadow-emerald-950/40 backdrop-blur-xl space-y-4">
              ...
              <div className="flex items-center justify-between pb-3 border-b border-slate-800 text-xs">
                ...
                <span className="text-[10px] bg-emerald-950 text-emerald-300 px-2.5 py-0.5 rounded-full border border-emerald-800 font-mono font-semibold">
                  Student Portal Live
                </span>
              </div>

              {/* 1. HERO FEATURE: Voice Ordering (LX) Widget */}
              <div
                onClick={onOpenVoiceModal}
                className="bg-slate-950 border border-emerald-500/50 rounded-2xl p-4 space-y-3 hover:border-emerald-400 transition-all cursor-pointer group shadow-lg shadow-emerald-500/10"
              >
              ...
              <div className="space-y-2">
                <div className="relative">
                  ...
                  <input
                    type="text"
                    readOnly
                    value="Search Biryani, Burgers, Dosa, Juices..."
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-8 pr-3 py-1.5 text-[11px] text-slate-400 cursor-default"
                  />
                </div>
                
                {/* Categories */}
                <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar text-[10px]">
                  <span className="bg-emerald-950 text-emerald-300 font-bold px-2.5 py-1 rounded-full border border-emerald-800 shrink-0">
                    🔥 Popular
                  </span>
                  <span className="bg-slate-950 text-slate-300 px-2.5 py-1 rounded-full border border-slate-800 shrink-0">
                    🍗 Biryani
                  </span>
                  <span className="bg-slate-950 text-slate-300 px-2.5 py-1 rounded-full border border-slate-800 shrink-0">
                    🍔 Fast Food
              ...
              {/* 3. Popular Food Items Photography Grid */}
              <div className="grid grid-cols-2 gap-2.5">
                
                {/* Food Item 1: Chicken Biryani */}
                <div className="bg-slate-950 border border-slate-800 rounded-2xl p-2.5 space-y-2 hover:border-emerald-500/40 transition-colors">
                  ...
                  <div className="flex items-center justify-between text-xs">
                    <div>
                      <h5 className="font-bold text-white text-[11px] truncate max-w-[100px]">Chicken Biryani</h5>
                      <p className="text-[9px] text-slate-400">Counter B • 8 mins</p>
                    </div>
                    <span className="font-mono text-emerald-400 font-bold text-[11px]">₹160</span>
                  </div>
                </div>

                {/* Food Item 2: Veg Burger */}
                <div className="bg-slate-950 border border-slate-800 rounded-2xl p-2.5 space-y-2 hover:border-emerald-500/40 transition-colors">
                  <div className="relative h-20 rounded-xl overflow-hidden bg-slate-900">
                    <img
                    ...
                    />
                    <span className="absolute top-1.5 right-1.5 bg-slate-950/80 backdrop-blur-md text-amber-300 px-1.5 py-0.5 rounded-full text-[9px] font-bold flex items-center gap-0.5">
                      <Star className="w-2.5 h-2.5 fill-amber-300 text-amber-300" /> 4.8
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-xs">
                    <div>
                      <h5 className="font-bold text-white text-[11px] truncate max-w-[100px]">Cheese Veg Burger</h5>
                      <p className="text-[9px] text-slate-400">Counter A • 6 mins</p>
                    </div>
                    <span className="font-mono text-emerald-400 font-bold text-[11px]">₹110</span>
                  </div>
                </div>

              </div>

              {/* 4. Cart Preview & Live Order Tracking Bar */}
              <div className="bg-slate-950 border border-slate-800 rounded-2xl p-3 space-y-2">
                <div className="flex items-center justify-between text-xs font-mono">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="text-white font-bold text-[11px] block">1 Item in Cart</span>
                      <span className="text-slate-400 text-[10px]">Chicken Biryani • ₹160</span>
                    </div>
                  </div>

                  <div className="text-right">
                    <span className="inline-flex items-center gap-1 text-[10px] bg-emerald-950 text-emerald-300 px-2 py-0.5 rounded-full border border-emerald-800 font-mono font-bold">
                ...
                <div className="space-y-1 pt-1 border-t border-slate-900">
                  <div className="flex justify-between text-[10px] text-slate-400 font-mono">
                    <span className="flex items-center gap-1">
                      ...
                      <span>Kitchen Prep Progress</span>
                    </span>
