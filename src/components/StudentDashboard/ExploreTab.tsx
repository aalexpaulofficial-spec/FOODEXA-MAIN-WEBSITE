...
const getCanteenName = (item: MenuItem, canteens: Canteen[]): string => {
  if (item.canteen_id) {
    const canteen = canteens.find(c => c.id === item.canteen_id);
    if (canteen) return canteen.name;
  }
  return item.counter_name || 'Canteen';
};

const getCanteenOrderingEnabled = (item: MenuItem, canteens: Canteen[]): boolean => {
  if (item.canteen_id) {
    const canteen = canteens.find(c => c.id === item.canteen_id);
    if (canteen) return canteen.is_ordering_enabled;
  }
  return true;
};
...
export const ExploreTab: React.FC<ExploreTabProps> = ({
  ...
  }, [filteredItems, activeCategory, dietaryFilter, favoritedIds]);

  const popularCanteens = useMemo(() => {
    if (!menuItems.length || !canteens.length) return [];

    const canteenItemMap = new Map<string, MenuItem[]>();
    menuItems.forEach(item => {
      if (item.canteen_id) {
        const arr = canteenItemMap.get(item.canteen_id) || [];
        arr.push(item);
        canteenItemMap.set(item.canteen_id, arr);
      }
    });

    return canteens
      .filter(c => canteenItemMap.has(c.id))
      .map(canteen => {
        const items = canteenItemMap.get(canteen.id) || [];
        const featuredItem = [...items]
          .sort((a, b) => Number(b.popular) - Number(a.popular) || b.rating - a.rating)[0];
        const avgRating = items.reduce((s, i) => s + i.rating, 0) / Math.max(items.filter(i => i.rating > 0).length, 1);
        const allPrepTimes = items.filter(i => i.prep_time_minutes).map(i => i.prep_time_minutes!);
        const avgPrepTime = allPrepTimes.length > 0
          ? Math.round(allPrepTimes.reduce((s, t) => s + t, 0) / allPrepTimes.length)
          : canteen.prep_time_minutes;
        return {
          canteen,
          featuredItem,
      ...
      .sort((a, b) => b.canteen.rating - a.canteen.rating || b.itemCount - a.itemCount)
      .slice(0, 6);
  }, [menuItems, canteens]);

  const getCanteenDistance = (canteen: Canteen): string => {
    if (canteen.location) return canteen.location;
    return 'Nearby';
  };
    ...
    <div className="flex-1 overflow-y-auto scroll-smooth pb-64">
      <div className="p-4 space-y-5 max-w-4xl mx-auto">
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <p className="text-xs sm:text-sm text-white/80 mt-1 max-w-lg leading-relaxed">
                  {currentBannerData.subtitle || `${institutionName || 'Your'} Campus Food Service`}
                </p>
              </div>

              <div className="absolute right-4 top-1/2 -translate-y-1/2 text-7xl sm:text-8xl select-none drop-shadow-lg opacity-80 pointer-events-none">
        ...
        {menuItems.length > 0 && (() => {
          ...
          const canteenName = recommended ? getCanteenName(recommended, canteens) : '';
          return recommended ? (
            <div className="w-full my-6">
          ...
          <div className="w-full my-6">
            ...
            <div className="flex gap-4 overflow-x-auto pb-3 scrollbar-none scroll-smooth">
              {trendingItems.map((item, index) => {
                const canteenName = getCanteenName(item, canteens);
                return (
                  <motion.div
                    key={item.id}
                    whileHover={{ y: -4 }}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {displayItems.map(item => (
                <FoodCard
                  ...
                  canteenName={getCanteenName(item, canteens)}
                  canteenOrderingEnabled={getCanteenOrderingEnabled(item, canteens)}
                />
              ))}
            </div>
          )}
        ...
        {popularCanteens.length > 0 && (
          <div className="w-full my-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {popularCanteens.map(({ canteen, featuredItem, itemCount, avgRating, avgPrepTime }) => {
                const orderingEnabled = canteen.is_ordering_enabled;
                return (
                  <motion.div
