    ...
    return (
      <div className="flex-1 overflow-y-auto pb-32">
        <div className="p-4 max-w-2xl mx-auto">
          ...
          <div className="rounded-[24px] border border-dashed border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 p-12 text-center">
            <Receipt className="w-12 h-12 mx-auto text-slate-300 dark:text-slate-600 mb-4" />
            <p className="text-sm font-bold text-slate-500 dark:text-slate-400">No orders yet</p>
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1 mb-5">Start ordering from the campus menu</p>
            <button
              onClick={onGoExplore}
    ...
    <div className="flex-1 overflow-y-auto pb-32 relative">
      <div className="p-4 space-y-4 max-w-2xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-black text-[#1D1D1F] dark:text-slate-100">Order History</h2>
          ...
          {pastOrders.slice(0, 30).map((order, idx) => {
            ...
            return (
              <motion.div
                ...
                className="rounded-[16px] border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden hover:border-blue-200 dark:hover:border-blue-800 hover:shadow-md transition-all duration-200"
              >
                {/* Card Header */}
                <div className="px-4 py-3 flex items-start justify-between border-b border-slate-100 dark:border-slate-800">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                      isCompleted ? 'bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800' : isCancelled ? 'bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800' : 'bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700'
                    }`}>
                    ...
                    <div>
                      <h3 className="font-black text-slate-900 dark:text-slate-100 text-sm">{orderNumber}</h3>
                ...
                <div className="px-4 py-3 space-y-2">
                  ...
                  <div className="space-y-1">
                    {order.items.slice(0, 3).map((item, i) => (
                      <div key={i} className="flex justify-between items-center text-xs">
                        <span className="text-slate-900 dark:text-slate-100 font-semibold truncate flex-1 pr-2">{item.quantity}x {item.name}</span>
                        <span className="text-slate-900 dark:text-slate-100 font-bold shrink-0">{formatINR(item.price * item.quantity)}</span>
                      </div>
                    ))}
                    {order.items.length > 3 && (
                      <p className="text-[10px] text-slate-400 font-medium">+{order.items.length - 3} more item{order.items.length - 3 > 1 ? 's' : ''}</p>
                    )}
                  </div>
                  ...
                  {(pickupCode || tokenNumber) && (
                    <div className="flex gap-2 mt-1">
                      {pickupCode && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg bg-blue-50 dark:bg-blue-950/30 text-[10px] font-bold text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-900/40">
                          <Hash className="w-2.5 h-2.5" /> {pickupCode}
                        </span>
                      )}
                      {tokenNumber && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg bg-amber-50 dark:bg-amber-950/30 text-[10px] font-bold text-amber-600 dark:text-amber-400 border border-amber-100 dark:border-amber-900/40">
                          Token: {tokenNumber}
                        </span>
                  )}
                </div>
              ...
              {isCancelled && (order.cancelled_by || order.cancelled_at) && (
                <div className="mt-2 p-2 rounded-lg bg-red-50 dark:bg-red-950/30 border border-red-100 dark:border-red-900/40 flex items-center gap-2 text-[11px] text-red-600 dark:text-red-400">
                ...
                <div className="px-4 py-3 bg-slate-50/50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-lg font-black text-slate-900 dark:text-slate-100">{formatINR(order.total_amount)}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => setDetailsOrder(order)}
                      className="flex items-center gap-1 px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs font-bold text-slate-900 dark:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
                    >
